...
class OrderSearchViewSet(viewsets.ViewSet):
    ...
    def search(self, request):
        ...
        user = request.user       
        orders = self.perform_search(request, user=user, store=store)        
    ...
    def perform_search(self,request, user, store):
        ...
        if q is None:            
            ...
            return Order.objects.filter(
                store_id=store_id, user=user, status=status).order_by('-created_at')[:30]
            ...
            return Order.objects.filter(                            
                ...
                Q(id__icontains=q),                   
                store_id=store_id, user=user, status=status
            )